...
import {IRecordData} from "../../../utils/RecordTypes.ts";
...
const RecordsChart: FC<RecordsChartProps> = ({records, user}) => {
    ...
    const data: IRecordData[] = records.map(record => ({
        date: record.date,
        value: record.currentWeight,
    }));

    return (
        <ResponsiveContainer width="100%" height={500} className={classes.recordsChart}>
            <LineChart data={data}>
                ...
                <YAxis
                    ...
                    stroke="#A0B5BB"
                />
                <Tooltip cursor={{stroke: "#F5F7F9"}} content={<CustomChartTooltip />}/>
                <Line
                    type="monotone"
                    dataKey="value"
                    stroke="#6CD2B0"
