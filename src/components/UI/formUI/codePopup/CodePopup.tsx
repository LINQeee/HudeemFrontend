...
interface CodePopupProps {
    ...
    userCredentials?: Omit<ICredentials, "code" | "authToken">;
...
const CodePopup: FC<CodePopupProps> = ({popupVisible, userCredentials, onSuccessSubmit}) => {
    ...
    const codeSubmit = async (value: string) => {
        try {
        const response = await checkCodeTrigger({code: value, email: userCredentials.email, rememberMe: userCredentials.rememberMe}).unwrap();
        onSuccessSubmit(response.authToken, response.email);
        }
        catch (err) {
            setCodeWrong(true);
        }
